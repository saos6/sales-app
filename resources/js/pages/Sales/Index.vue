...
watch(() => form.per_page, (newValue) => {
  router.get(route('sales.index'), { ...route().params, per_page: newValue }, { preserveState: true });
});
...
const columns = reactive([
  { key: 'sales_no', label: '売上番号', visible: true, sortable: true },
  { key: 'sales_date', label: '売上日', visible: true, sortable: true },
...
const sortBy = (key: string) => {
  ...
  router.get(route('sales.index'), { ...route().params, sort: key, order: newOrder }, { preserveState: true });
};
          ...
          <div class="p-6 text-gray-900 dark:text-gray-100">
            <div class="mb-4">
              <form @submit.prevent="search">
                <div class="flex gap-2">
                  <Input v-model="form.search" placeholder="売上番号、件名、顧客名で検索..." class="max-w-xs" />
            ...
            <div class="flex justify-between items-center mb-4">
              <div class="flex gap-2">
                ...
                <a :href="route('sales.export', { search: form.search })">
            ...
            <div v-if="sales.meta" class="text-sm text-gray-500 mb-4">
              {{ sales.meta.total }}件見つかりました。
            </div>

            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead v-if="isVisible('sales_no')" @click="sortBy('sales_no')" class="cursor-pointer">
                    <div class="flex items-center">
                      売上番号 <component :is="sortIcon('sales_no')" class="ml-2 h-4 w-4" />
                    </div>
                  </TableHead>
                  <TableHead v-if="isVisible('sales_date')" @click="sortBy('sales_date')" class="cursor-pointer">
                     <div class="flex items-center">
                      売上日 <component :is="sortIcon('sales_date')" class="ml-2 h-4 w-4" />
                    </div>
                  </TableHead>
                  ...
                  <TableHead v-if="isVisible('subject')" @click="sortBy('subject')" class="cursor-pointer">
                     <div class="flex items-center">
                      件名 <component :is="sortIcon('subject')" class="ml-2 h-4 w-4" />
                    </div>
                  </TableHead>
                  <TableHead v-if="isVisible('total')" @click="sortBy('total')" class="cursor-pointer">
                     <div class="flex items-center">
                      合計金額 <component :is="sortIcon('total')" class="ml-2 h-4 w-4" />
                    </div>
                  </TableHead>
              ...
              <TableBody>
                <TableRow v-for="sale in sales.data" :key="sale.id">
                  <TableCell v-if="isVisible('sales_no')">{{ sale.sales_no }}</TableCell>
                  <TableCell v-if="isVisible('sales_date')">{{ sale.sales_date }}</TableCell>
                  <TableCell v-if="isVisible('cust_name')">{{ sale.cust_name }}</TableCell>
                  <TableCell v-if="isVisible('subject')">{{ sale.subject }}</TableCell>
                  <TableCell v-if="isVisible('total')">{{ sale.total }}</TableCell>
                  <TableCell v-if="isVisible('status')">{{ sale.status }}</TableCell>
                  <TableCell v-if="isVisible('actions')">
                    <Link :href="route('sales.edit', sale.id)" class="mr-2">
                      <Button variant="outline" size="sm">編集</Button>
                    </Link>
                    <Button variant="destructive" size="sm" @click="deleteSale(sale.id)">削除</Button>
                  </TableCell>
                </TableRow>
              </TableBody>
            </Table>

            <div v-if="sales.links" class="mt-4 flex justify-center">
